...
describe('social link interaction styles', () => {
    ...
    it('uses the glass-lens hover lift for custom social links', () => {
        expect(componentsCss).toMatch(
            /\.social-link\s*\{[\s\S]*transition:\s*transform\s+0\.5s\s+var\(--curve-lift\),[\s\S]*box-shadow\s+0\.8s\s+var\(--curve-delicate\),[\s\S]*background-color\s+0\.6s\s+var\(--curve-snap\),[\s\S]*border-color\s+0\.6s\s+var\(--curve-snap\);/
        );
        expect(componentsCss).toMatch(
            /\.social-link-slot[\s\S]*?\.social-link--custom\s*\{[\s\S]*?transform:\s*var\(--lift-sm\);[\s\S]*?box-shadow:[\s\S]*?0 0 70px 10px color-mix\(in srgb, var\(--custom-color,\s*#ffe600\) 12%, transparent\),[\s\S]*?0 0 36px 4px color-mix\(in srgb, var\(--custom-color,\s*#ffe600\) 22%, transparent\),[\s\S]*?0 0 14px 2px color-mix\(in srgb, var\(--custom-color,\s*#ffe600\) 38%, transparent\);/
        );
    });
